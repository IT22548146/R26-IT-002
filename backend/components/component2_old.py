...
def _build_features(
    ...
    cap_pen  = max(0.0, (sp_util - 60.0) / 40.0)
    cap_mult = round(1.0 + cap_pen * 0.40, 6)
...
def predict():
    ...
    daily_commit_ood       = not (DAILY_COMMIT_ADJ_MIN_TRAIN
                                  <= _daily_adj_precheck
                                  <= DAILY_COMMIT_ADJ_MAX_TRAIN)
    if daily_commit_ood:
    ...
    try:
        ...
        # ── Hybrid: three decision paths ───────────────────────────
        #
        # A) OOD daily_commitment_adj — ML is extrapolating outside its training
        #    range. Embroidery prediction is unreliable. Use formula-derived
        #    embroidery; keep raw M1/M2 for cutting/sewing (they are reliable
        #    even OOD because cutting/sewing depend mainly on order_qty and design,
        #    not on stitch-count throughput).
        #
        # B) ML total > formula_base — complexity drives days above the capacity
        #    floor. ML is reliable; scale all three components proportionally.
        #
        # C) Formula wins — ML total is below the capacity floor. Using the
        #    old scaled-ML breakdown would compress embroidery DOWN even as the
        #    total goes UP (the 200→100 daily_commitment bug). Instead use
        #    formula-derived embroidery so the breakdown matches the workload.

        if daily_commit_ood:
            # Case A
            days_driver        = "formula (OOD: daily_commitment outside training range)"
            _formula_breakdown = True
        elif ml_total > formula_base:
            # Case B
            days_driver        = "ML (complexity)"
            _formula_breakdown = False
        else:
            # Case C
            days_driver        = "formula (capacity)"
            _formula_breakdown = True

        if _formula_breakdown:
            # Embroidery from the formula; cutting/sewing direct from ML.
            # Total = actual arithmetic sum of the three components.
            ...
            # Case B: ML wins — scale all three to the rounded ML total.
            total_prod_days_int = math.ceil(ml_total)
            scale               = total_prod_days_int / ml_total if ml_total > 0 else 1.0
        ...
        return jsonify({
            ...
            "warnings":   [w for w in [stitch_warning] if w],
            "derived": {
